...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Karna doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in karna_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `karna model` uses — instead of
...
export type KarnaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `karna model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Karna desktop custom model modality, e.g. chat/image/embedding. */
  model_type?: string
  ...
  /** Karna only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Karna only: paid models a free-tier user cannot select (shown disabled). */
...
export interface CustomModelInfo {
  ...
  base_url: string
  model_name: string
  type?: 'chat' | 'vision' | 'image' | 'embedding' | 'rerank' | 'speech' | 'transcription' | string
  endpoint?: string
  enabled: boolean
  usable: boolean
  provider: string
  api_key_env: string
  api_key_set: boolean
  api_key_redacted?: null | string
  last_test?: null | { ok?: boolean; message?: string; tested_at?: string; status?: number; sample?: string }
}
...
/** Shape of `GET /api/karna/update/check` — the backend's own update state.
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Karna Tool Gateway as a result of
   *  switching the main provider to Karna. Empty unless provider === 'karna'
