...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/karna'
...
import { type AppView, ARTIFACTS_ROUTE, SETTINGS_ROUTE, SKILLS_ROUTE, SOUL_ROUTE, WORKFLOWS_ROUTE, WRITER_ROUTE } from '../../routes'
...
interface WriterProjectTask { id: string; status: string; owner_agent_id?: string }
interface WriterProjectInfo {
  id: string
  title: string
  slug: string
  folder: string
  ...
  agents?: WriterProjectAgent[]
  tasks?: { tasks?: WriterProjectTask[] }
  pinned?: boolean
}
...
const SIDEBAR_NAV: SidebarNavItem[] = [
  ...
  },
  {
    id: 'new-project',
    label: '\u65b0\u5efa\u9879\u76ee',
    icon: props => <Codicon name="new-folder" {...props} />,
    action: 'new-project'
  },
  {
    id: 'skills',
  ...
  { id: 'workflows', label: '\u591a Agent \u5de5\u574a', icon: props => <Codicon name="type-hierarchy-sub" {...props} />, route: WORKFLOWS_ROUTE },
  { id: 'writer', label: '\u4f5c\u54c1\u5de5\u574a', icon: props => <Codicon name="notebook" {...props} />, route: WRITER_ROUTE },
  { id: 'soul', label: '\u7075\u9b42\u5de5\u574a', icon: props => <Codicon name="sparkle" {...props} />, route: SOUL_ROUTE },
...
export function ChatSidebar({
  ...
  const [writerProjects, setWriterProjects] = useState<WriterProjectInfo[]>([])
  ...
  useEffect(() => {
    ...
    window.addEventListener('karna:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('karna:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])

  const refreshWriterProjects = useCallback(async () => {
    try {
      const result = await window.karnaDesktop.api<{ projects?: WriterProjectInfo[] }>({ path: '/api/writer/projects' })
      const projects = Array.isArray(result.projects) ? result.projects : []
      setWriterProjects(projects)
      setOpenProjectIds(current => ({ ...Object.fromEntries(projects.map(project => [project.id, current[project.id] ?? true])) }))
    } catch {
      setWriterProjects([])
    }
  }, [])

  useEffect(() => { void refreshWriterProjects() }, [refreshWriterProjects, projectRefreshTick, sessions.length])
  useEffect(() => { const timer = window.setInterval(() => void refreshWriterProjects(), 3000); return () => window.clearInterval(timer) }, [refreshWriterProjects])

  const createProjectSession = useCallback(async (project: WriterProjectInfo, agent?: WriterProjectAgent) => {
    try {
      const result = await window.karnaDesktop.api<{ ok: boolean; error?: string; session?: { id: string } }>({
        path: `/api/writer/projects/${encodeURIComponent(project.id)}/sessions`,
        method: 'POST',
        body: { agentId: agent?.id, title: agent ? `${agent.name} \u00b7 ${project.title}` : `${project.title} \u00b7 \u65b0\u5bf9\u8bdd` }
      })
      if (!result.ok || !result.session?.id) throw new Error(result.error || '创建项目会话失败')
      await refreshWriterProjects()
      onResumeSession(result.session.id)
    } catch (err) {
      notifyError(err, '创建项目会话失败')
    }
  }, [onResumeSession, refreshWriterProjects])

  const handleProjectAction = useCallback(async (project: WriterProjectInfo, action: string) => {
    try {
      if (action === 'rename') {
        const title = window.prompt('\u91cd\u547d\u540d\u9879\u76ee', project.title)?.trim()
        if (!title) return
        await window.karnaDesktop.api({ path: `/api/writer/projects/${encodeURIComponent(project.id)}`, method: 'PATCH', body: { title } })
      } else if (action === 'archive') {
        await window.karnaDesktop.api({ path: `/api/writer/projects/${encodeURIComponent(project.id)}`, method: 'PATCH', body: { status: 'archived' } })
      } else if (action === 'delete') {
        if (!window.confirm(`\u5220\u9664\u9879\u76ee\u201c${project.title}\u201d\uff1f\u9879\u76ee\u6587\u4ef6\u5939\u548c\u9879\u76ee\u4f1a\u8bdd\u4e5f\u4f1a\u5220\u9664\u3002`)) return
        await window.karnaDesktop.api({ path: `/api/writer/projects/${encodeURIComponent(project.id)}`, method: 'DELETE' })
      } else if (action === 'open-folder') {
        await window.karnaDesktop.api({ path: `/api/writer/projects/${encodeURIComponent(project.id)}/open-folder`, method: 'POST' })
      } else if (action === 'pin') {
        await window.karnaDesktop.api({ path: `/api/writer/projects/${encodeURIComponent(project.id)}`, method: 'PATCH', body: { pinned: !project.pinned } })
      } else if (action === 'active') {
        await window.karnaDesktop.api({ path: `/api/writer/projects/${encodeURIComponent(project.id)}/open`, method: 'POST' })
      }
      await refreshWriterProjects()
    } catch (err) {
      notifyError(err, '\u9879\u76ee\u64cd\u4f5c\u5931\u8d25')
    }
  }, [refreshWriterProjects])
  ...
  const projectSessionIdSet = useMemo(() => new Set(writerProjects.flatMap(project => project.session_ids || []).concat(visibleSessions.filter(session => session.project_id).map(session => session.id))), [writerProjects, visibleSessions])
  const nonProjectSortedSessions = useMemo(() => sortedSessions.filter(session => !session.project_id && !projectSessionIdSet.has(session.id)), [sortedSessions, projectSessionIdSet])
  ...
  const pinnedSessions = useMemo(() => {
    ...
    for (const pinId of pinnedSessionIds) {
      ...
      if (session && !session.project_id && !seen.has(session.id)) {
        seen.add(session.id)
        out.push(session)
      }
    }
  ...
  const unpinnedAgentSessions = useMemo(
    () => nonProjectSortedSessions.filter(s => !pinnedRealIdSet.has(s.id)),
    [nonProjectSortedSessions, pinnedRealIdSet]
  )
  ...
  const showSessionSkeletons = sessionsLoading && sortedSessions.length === 0 && writerProjects.length === 0

  const showSessionSections = showSessionSkeletons || sortedSessions.length > 0 || writerProjects.length > 0
      ...
      <SidebarContent className="gap-0 overflow-hidden bg-transparent px-2.5">
            ...
            <SidebarMenu className="gap-px">
              {SIDEBAR_NAV.map(item => {
                ...
                const active =
                  ...
                  (item.id === 'writer' && currentView === 'writer') ||
                  (item.id === 'soul' && currentView === 'soul')

                const isNewSession = item.id === 'new-session'
                const isNewProject = item.id === 'new-project'
        ...
        {contentVisible && showSessionSections && (
          <div className={cn('flex min-h-0 flex-1 flex-col pb-1.75', SCROLL_Y)}>
            ...
            {!trimmedQuery && writerProjects.length > 0 && (
              <SidebarProjectTree
                activeSessionId={activeSidebarSessionId}
                ...
                onToggleProject={projectId => setOpenProjectIds(current => ({ ...current, [projectId]: !(current[projectId] ?? true) }))}
                open={projectsOpen}
...
function taskSummary(project: WriterProjectInfo) {
  const tasks = project.tasks?.tasks || []
  if (!tasks.length) return '0'
  const done = tasks.filter(task => task.status === 'done').length
  const active = tasks.filter(task => ['claimed', 'in_progress', 'review'].includes(task.status)).length
  return `${done}/${tasks.length}${active ? ` \u00b7 ${active}\u8fdb\u884c\u4e2d` : ''}`
}


function agentStatusDotClass(status?: string) {
  if (status === 'working' || status === 'in_progress') return 'bg-emerald-500'
  if (status === 'waiting') return 'bg-amber-400'
  if (status === 'pending_approval' || status === 'review') return 'bg-violet-500'
  if (status === 'blocked') return 'bg-red-500'
  return 'bg-(--ui-text-quaternary)'
}

function projectSessionLabel(session: SessionInfo) {
  const raw = session.agent_name === 'Project Controller' ? '\u4e3b\u63a7' : (session.agent_name || session.title || '\u9879\u76ee\u5bf9\u8bdd')
  return raw.replace(/Project Controller/g, '\u4e3b\u63a7').replace(/ \? /g, ' \u00b7 ').replace(/\?\s*\u65b0\u5bf9\u8bdd/g, '\u00b7 \u65b0\u5bf9\u8bdd')
}
...
function SidebarProjectTree({
  ...
  onCreateSession: (project: WriterProjectInfo, agent?: WriterProjectAgent) => void
  onResumeSession: (sessionId: string) => void
  onProjectAction: (project: WriterProjectInfo, action: string) => void
  onArchiveSession: (sessionId: string) => void
  onDeleteSession: (sessionId: string) => void
  onToggle: () => void
  ...
  projects: WriterProjectInfo[]
  sessions: SessionInfo[]
  workingSessionIdSet: Set<string>
}) {
  const [menu, setMenu] = useState<null | { x: number; y: number; project?: WriterProjectInfo; session?: SessionInfo }>(null)
  ...
  const agentBySession = useMemo(() => {
    const rows = new Map<string, WriterProjectAgent>()
    for (const project of projects) for (const agent of project.agents || []) if (agent.session_id) rows.set(agent.session_id, agent)
    return rows
  }, [projects])
  const renderMiniSession = (session: SessionInfo, indent = false) => (
    <button
      className={cn('group relative flex min-h-[1.625rem] w-full items-center gap-1.5 rounded-md px-2 py-0.5 text-left text-[0.8125rem] hover:bg-(--ui-row-hover-background)', indent && 'pl-7', activeSessionId === session.id && 'bg-(--ui-row-active-background)')}
      key={session.id}
      onClick={() => onResumeSession(session.id)}
      onContextMenu={event => { event.preventDefault(); setMenu({ x: event.clientX, y: event.clientY, session }) }}
      type="button"
    >
      <span className={cn('size-1.5 rounded-full', workingSessionIdSet.has(session.id) ? 'bg-(--ui-accent)' : agentStatusDotClass(agentBySession.get(session.id)?.status))} />
      <span className="min-w-0 flex-1 truncate text-(--ui-text-secondary)">{projectSessionLabel(session)}</span>
    </button>
  )

  return (
    <SidebarGroup className="shrink-0 p-0 pb-1">
      <SidebarSectionHeader icon={<Codicon name="repo" size="0.75rem" />} label={'\u9879\u76ee'} meta={String(projects.length)} onToggle={onToggle} open={open} />
      {open && <SidebarGroupContent className="grid gap-px pb-2 pt-1">
        {projects.map(project => {
          const projectOpen = openProjectIds[project.id] ?? true
          const projectSessions = (project.session_ids || []).map(id => byId.get(id)).filter(Boolean) as SessionInfo[]
          const mainSession = project.main_session_id ? byId.get(project.main_session_id) : projectSessions.find(session => !session.agent_id)
          const ordinarySessions = projectSessions.filter(session => session.id !== mainSession?.id && !session.agent_id)
          return (
            <div className="grid gap-px" key={project.id}>
              <div className="group/project flex min-h-7 items-center gap-1 rounded-md px-2 hover:bg-(--ui-row-hover-background)" onContextMenu={event => { event.preventDefault(); setMenu({ x: event.clientX, y: event.clientY, project }) }}>
                <button className="grid size-4 place-items-center text-(--ui-text-tertiary)" onClick={() => onToggleProject(project.id)} type="button"><DisclosureCaret open={projectOpen} /></button>
                <button className="min-w-0 flex-1 truncate text-left text-[0.8125rem] font-medium text-(--ui-text-secondary)" onClick={() => onToggleProject(project.id)} type="button">{project.title}</button>
                <span className="text-[0.625rem] text-(--ui-text-quaternary)">{taskSummary(project)}</span>
                <button className="grid size-5 place-items-center rounded-sm text-(--ui-text-tertiary) opacity-0 hover:bg-(--ui-control-hover-background) group-hover/project:opacity-100" onClick={event => { event.stopPropagation(); onCreateSession(project) }} type="button"><Codicon name="add" size="0.75rem" /></button>
              </div>
              {projectOpen && <div className="grid gap-px">
                {mainSession && renderMiniSession({ ...mainSession, title: '\u4e3b\u63a7', agent_name: '\u4e3b\u63a7' }, true)}
                {(project.multi_agent_enabled ? (project.agents || []) : []).filter(agent => agent.enabled !== false && agent.id !== 'controller').map(agent => {
                  const session = agent.session_id ? byId.get(agent.session_id) : null
                  return session ? renderMiniSession(session, true) : (
                    <button className="flex min-h-[1.625rem] items-center gap-1.5 rounded-md py-0.5 pl-7 pr-2 text-left text-[0.8125rem] hover:bg-(--ui-row-hover-background)" key={agent.id} onClick={() => onCreateSession(project, agent)} type="button"><Codicon name="person" size="0.75rem" /><span className="min-w-0 flex-1 truncate">{agent.name}</span><span className="text-[0.625rem] text-(--ui-text-quaternary)">创建会话</span></button>
                  )
                })}
                {ordinarySessions.map(session => renderMiniSession(session, true))}
              </div>}
            </div>
          )
        })}
      ...
      {menu && (
        <div className="fixed z-[300] min-w-40 overflow-hidden rounded-lg border border-slate-300 bg-white p-1 text-xs text-slate-900 shadow-2xl" style={{ left: menu.x, top: menu.y }}>
          {menu.project && ([['active', '\u8bbe\u4e3a\u5f53\u524d\u9879\u76ee'], ['pin', menu.project.pinned ? '\u53d6\u6d88\u7f6e\u9876' : '\u7f6e\u9876\u9879\u76ee'], ['open-folder', '\u6253\u5f00\u9879\u76ee\u6587\u4ef6\u5939'], ['rename', '\u91cd\u547d\u540d\u9879\u76ee'], ['archive', '\u5f52\u6863\u9879\u76ee'], ['delete', '\u5220\u9664\u9879\u76ee']] as const).map(([action, label]) => (
            <button className={cn('block w-full rounded-md px-2 py-1.5 text-left hover:bg-slate-100', action === 'delete' && 'text-red-600')} key={action} onClick={() => { const project = menu.project; if (!project) return; setMenu(null); onProjectAction(project, action) }} type="button">{label}</button>
          ))}
          {menu.session && ([['open', '\u6253\u5f00\u5bf9\u8bdd'], ['archive', '\u5f52\u6863\u5bf9\u8bdd'], ['delete', '\u5220\u9664\u5bf9\u8bdd']] as const).map(([action, label]) => (
            <button className={cn('block w-full rounded-md px-2 py-1.5 text-left hover:bg-slate-100', action === 'delete' && 'text-red-600')} key={action} onClick={() => { const session = menu.session; if (!session) return; setMenu(null); if (action === 'open') onResumeSession(session.id); if (action === 'archive') onArchiveSession(session.id); if (action === 'delete') onDeleteSession(session.id) }} type="button">{label}</button>
          ))}
        </div>
      )}
    </SidebarGroup>
