...
import { karnaDirectiveFormatter, type SlashChipKind } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, KARNA_PATHS_MIME, partitionDroppedFiles } from '../hooks/use-composer-actions'
...
interface ComposerResourceRow { id: string; name: string; description?: string }
interface ComposerWorkflowRow { id: string; name: string; nodes?: unknown[] }
...
export function ChatBar({
  ...
  // Distinguish a cold start ("Starting Karna...") from a dropped connection
  ...
  const [karnaMode, setKarnaMode] = useState<'chat' | 'plan' | 'goal'>('chat')
  const [resourceOpen, setResourceOpen] = useState(false)
  const [composerResources, setComposerResources] = useState<{ skills: ComposerResourceRow[]; mcp: ComposerResourceRow[]; knowledge: ComposerResourceRow[]; workflows: ComposerWorkflowRow[] }>({ skills: [], mcp: [], knowledge: [], workflows: [] })
  const [selectedWorkflowId, setSelectedWorkflowId] = useState('')
  const [selectedResources, setSelectedResources] = useState<{ skills: string[]; mcp: string[]; knowledge: string[] }>({ skills: [], mcp: [], knowledge: [] })
  const [trigger, setTrigger] = useState<TriggerState | null>(null)
  const [triggerActive, setTriggerActive] = useState(0)
  const [triggerItems, setTriggerItems] = useState<readonly Unstable_TriggerItem[]>([])
  // Set synchrokarnaly in keydown when the open trigger popover consumes a
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = karnaDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, KARNA_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, KARNA_PATHS_MIME)) {
      return
    }
  ...
  const toggleComposerResource = (kind: 'skills' | 'mcp' | 'knowledge', name: string) => {
    setSelectedResources(current => ({ ...current, [kind]: current[kind].includes(name) ? current[kind].filter(item => item !== name) : [...current[kind], name] }))
  }
  const loadComposerResources = async () => {
    try {
      const result = await window.karnaDesktop.api<{ skills?: ComposerResourceRow[]; mcp?: ComposerResourceRow[]; knowledge?: ComposerResourceRow[] }>({ path: '/api/writer/resources' })
      const workflows = await window.karnaDesktop.api<{ workflows?: ComposerWorkflowRow[] }>({ path: '/api/writer/workflows' }).catch(() => ({ workflows: [] }))
      setComposerResources({ skills: result.skills || [], mcp: result.mcp || [], knowledge: result.knowledge || [], workflows: workflows.workflows || [] })
    } catch {}
  }
  const enhanceComposerPrompt = async () => {
    ...
    if (!text) return
    try {
      const result = await window.karnaDesktop.api<{ ok: boolean; text?: string; error?: string }>({ path: '/api/prompt/enhance', method: 'POST', body: { text, mode: karnaMode, skills: selectedResources.skills, mcp: selectedResources.mcp, knowledge: selectedResources.knowledge } })
      if (!result.ok || !result.text) throw new Error(result.error || 'enhance failed')
      loadIntoComposer(result.text, $composerAttachments.get())
      notify({ kind: 'success', title: '\u5df2\u589e\u5f3a\u63d0\u793a\u8bcd', message: '\u5df2\u5199\u5165\u8f93\u5165\u6846' })
    } catch (err) { notifyError(err, '\u63d0\u793a\u8bcd\u589e\u5f3a\u5931\u8d25') }
  }

  const formatKarnaModePrompt = (text: string) => {
    const workflowLine = selectedWorkflowId ? `[karna_workflow:${encodeURIComponent(selectedWorkflowId)}]\n` : ''
    const resourceLine = selectedResources.skills.length || selectedResources.mcp.length || selectedResources.knowledge.length
      ? `\n\u53ef\u7528\u8d44\u6e90\uff1a\u6280\u80fd=${selectedResources.skills.join('\u3001') || '\u65e0'}\uff1bMCP/\u5de5\u5177=${selectedResources.mcp.join('\u3001') || '\u65e0'}\uff1b\u77e5\u8bc6\u5e93=${selectedResources.knowledge.join('\u3001') || '\u65e0'}\n`
      : '\n'
    if (karnaMode === 'plan') return `${workflowLine}${resourceLine}\u8bf7\u5148\u7ed9\u51fa\u6e05\u6670\u8ba1\u5212\uff0c\u518d\u6267\u884c\u3002\u8ba1\u5212\u63a7\u5236\u5728 2-4 \u6b65\uff0c\u6bcf\u6b65\u8bf4\u660e\u4ea4\u4ed8\u7269\u3002\n\n${text}`
    if (karnaMode === 'goal') return `${workflowLine}${resourceLine}\u8bf7\u628a\u76ee\u6807\u62c6\u6210\u53ef\u6267\u884c\u4efb\u52a1\uff0c\u5e76\u5728\u6267\u884c\u8fc7\u7a0b\u4e2d\u6301\u7eed\u6c47\u62a5\u8fdb\u5c55\u3002\n\n${text}`
    return `${workflowLine}${resourceLine}${text}`
  }
  ...
  const dispatchSubmit = (text: string, attachments?: ComposerAttachment[]) => {
    ...
    const finalText = formatKarnaModePrompt(text)
    void Promise.resolve(attachments ? onSubmit(finalText, { attachments }) : onSubmit(finalText))
  ...
  const input = (
    <div className={cn('relative', stacked ? 'w-full' : 'min-w-(--composer-input-inline-min-width) flex-1')}>
      ...
      {/* assistant-ui requires ComposerPrimitive.Input somewhere in the tree
        ...
        forcing two synchrokarna layouts per keystroke for an element the
          ...
          <div className="relative w-full rounded-[inherit]">
            ...
            >
              ...
              >
                ...
                <div className="flex flex-wrap gap-1 px-1 text-[0.68rem]">
                  {(['chat', 'plan', 'goal'] as const).map(mode => (
                    <button className={cn('rounded-full px-2 py-0.5 text-muted-foreground hover:bg-accent/40', karnaMode === mode && 'bg-primary/12 text-primary')} key={mode} onClick={() => setKarnaMode(mode)} type="button">{mode === 'chat' ? '\u804a\u5929' : mode === 'plan' ? '\u8ba1\u5212\u6a21\u5f0f' : '\u76ee\u6807\u6a21\u5f0f'}</button>
                  ))}
                  <button className="rounded-full px-2 py-0.5 text-muted-foreground hover:bg-accent/40" onClick={() => void enhanceComposerPrompt()} type="button">{'\u63d0\u793a\u8bcd\u589e\u5f3a'}</button>
                  <button className={cn('rounded-full px-2 py-0.5 text-muted-foreground hover:bg-accent/40', resourceOpen && 'bg-primary/12 text-primary')} onClick={() => { setResourceOpen(v => !v); void loadComposerResources() }} type="button">{'\u8d44\u6e90'}</button>
                </div>
                {resourceOpen && <div className="grid gap-1 rounded-lg border border-border/70 bg-background/80 p-2 text-[0.68rem]"><div className="flex flex-wrap items-center gap-1"><span className="mr-1 text-muted-foreground">多Agent</span><button className={cn('rounded border px-1.5 py-0.5', !selectedWorkflowId ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground')} onClick={() => setSelectedWorkflowId('')} type="button">不使用</button>{composerResources.workflows.slice(0, 8).map(row => <button className={cn('rounded border px-1.5 py-0.5', selectedWorkflowId === row.id ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground')} key={row.id} onClick={() => setSelectedWorkflowId(row.id)} type="button">{row.name}</button>)}</div>{([['skills','\u6280\u80fd'], ['mcp','MCP/\u5de5\u5177'], ['knowledge','\u77e5\u8bc6\u5e93']] as const).map(([kind, label]) => <div className="flex flex-wrap items-center gap-1" key={kind}><span className="mr-1 text-muted-foreground">{label}</span>{composerResources[kind].slice(0, 8).map(row => <button className={cn('rounded border px-1.5 py-0.5', selectedResources[kind].includes(row.name) ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground')} key={row.id} onClick={() => toggleComposerResource(kind, row.name)} type="button">{row.name}</button>)}</div>)}</div>}
                <div
                  className={cn(
                    'grid w-full',
                    stacked
