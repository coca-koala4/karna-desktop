...
import { getKarnaConfig, getKarnaConfigDefaults } from '@/karna'
...
interface KarnaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useKarnaConfig({ activeSessionIdRef, refreshProjectBranch }: KarnaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshKarnaConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getKarnaConfig(), getKarnaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshKarnaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
