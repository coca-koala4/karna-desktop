...
interface KnowledgeStore { config?: Record<string, unknown>; libraries?: ResourceRow[] }
...
const EMPTY_STATE: WorkshopState = { authors: [], knowledge: [], knowledgeStore: {}, mcpServers: [], projects: [], resources: { knowledge: [], mcp: [], skills: [] }, workflowRuns: [], workflows: [] }
...
const VIEW_COPY: Record<WorkshopKind, { desc: string; icon: string; title: string }> = {
  ...
  writer: { desc: '管理作品项目、主控会话、Prompt 增强和项目级知识绑定。全局 RAG 管理已迁移到灵魂工坊。', icon: 'notebook', title: '作品工坊' },
  soul: { desc: '统一管理作者人格、文本样本、RAG 知识库、检索测试与安全融合。', icon: 'sparkle', title: '灵魂工坊' },
...
function useWorkshopData() {
  ...
  const load = useCallback(async () => {
    setRefreshing(true)
    try {
      const [projects, resources, workflows, authors, knowledge, mcp] = await Promise.all([
...
function AgentsView({ data }: { data: Data }) {
  ...
  const create = () => runAction('创建工作流失败', async () => { const res = await karnaApi<{ workflow?: WorkflowRow }>(`/api/writer/workflows?project=${encodeURIComponent(projectRef)}`, { method: 'POST', body: starter() }); if (res.workflow) setSelected(res.workflow.id); setName(''); await loadWorkflows() })
  const save = () => runAction('保存工作流 JSON 失败', async () => { const res = await karnaApi<{ workflow?: WorkflowRow }>(`/api/writer/workflows?project=${encodeURIComponent(projectRef)}`, { method: 'POST', body: JSON.parse(json) }); if (res.workflow) setSelected(res.workflow.id); setJson(''); await loadWorkflows() })
...
function McpView({ data }: { data: Data }) {
  ...
  const remove = (server: McpServer) => { if (builtinMcp(server)) return; if (!window.confirm(`确定删除 MCP 服务器「${server.name}」吗？`)) return; void runAction('删除 MCP 服务器失败', async () => { await karnaApi(`/api/mcp/servers/${encodeURIComponent(server.name)}`, { method: 'DELETE' }); await load() }) }
  const reload = () => runAction('重新加载 MCP 失败', async () => { setResult(asText(await karnaApi('/api/mcp/reload', { method: 'POST' }))); await load() })
  return <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]"><Card><div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">新增 MCP 服务器</h2><p className="mt-1 text-sm text-muted-foreground">支持 HTTP URL 或 stdio command。内置 Karna MCP 只读展示。</p></div><Button onClick={() => void reload()} size="sm" variant="outline">重新加载 MCP</Button></div><Input className="mt-3" onChange={e => setName(e.target.value)} placeholder="配置键，例如 local-search" value={name} /><Input className="mt-2" onChange={e => setDescription(e.target.value)} placeholder="描述，可选" value={description} /><Input className="mt-2" onChange={e => setUrl(e.target.value)} placeholder="HTTP URL，例如 http://127.0.0.1:3000/mcp" value={url} /><Input className="mt-2" onChange={e => setCommand(e.target.value)} placeholder="stdio 命令，例如 npx" value={command} /><Button className="mt-3" disabled={!name.trim() || (!url.trim() && !command.trim())} onClick={() => void save()} size="sm">保存服务器</Button>{result && <pre className="mt-3 max-h-64 overflow-auto whitespace-pre-wrap rounded-lg bg-muted p-3 text-xs">{result}</pre>}</Card><Card><h2 className="font-semibold">MCP 服务器列表</h2>{state.mcpServers.length ? <div className="mt-4 grid gap-2">{state.mcpServers.map(server => <div className="rounded-lg border border-border/60 p-3" key={server.name}><div className="font-medium">{server.name}</div><div className="mt-1 text-xs text-muted-foreground">{server.description || server.url || server.command || server.transport || '未配置描述'}</div><div className="mt-2 text-xs text-muted-foreground">{server.enabled === false ? '已禁用' : '已启用'} / 工具 {server.tools?.length || 0} 个{builtinMcp(server) ? ' / 内置只读' : ''}</div><div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => void test(server)} size="sm" variant="outline">测试</Button><Button disabled={builtinMcp(server)} onClick={() => void toggle(server)} size="sm" variant="ghost">{server.enabled === false ? '启用' : '禁用'}</Button><Button disabled={builtinMcp(server)} onClick={() => remove(server)} size="sm" variant="ghost">删除</Button></div>{server.tools?.length ? <div className="mt-3 flex flex-wrap gap-1 text-xs">{server.tools.map(tool => <span className="rounded bg-muted px-2 py-0.5" key={tool}>{tool}</span>)}</div> : null}</div>)}</div> : <EmptyPanel>还没有 MCP 服务器。</EmptyPanel>}</Card></div>
}
...
function KarnaWorkshopByKind({ kind }: { kind: WorkshopKind }) {
  ...
  if (data.loading) return <PageLoader label={`正在加载${VIEW_COPY[kind].title}…`} />
  return <Shell data={data} kind={kind}><McpView data={data} /></Shell>
}
