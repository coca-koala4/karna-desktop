#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Karna icon + version metadata onto the
// built Karna.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Karna.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Karna.exe>
//
...
// Stamp the Karna icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Karna',
      FileDescription: 'Karna Desktop',
      CompanyName: 'Karna',
      LegalCopyright: 'Copyright (c) 2026 Karna'
    }
  })

  console.log('[set-exe-identity] done — Karna icon + identity stamped')
}
