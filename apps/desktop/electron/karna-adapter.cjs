...
const sessionInfoPayload = session => ({
  ...session,
  cwd: karnaConfig?.terminal?.cwd || process.cwd(),
  running: Boolean(session?.running),
  desktop_contract: 1,
  personality: karnaConfig?.display?.personality || '',
...
const storedSessionInfo = session => ({
  ...
  is_project_session: Boolean(session.project_id)
})
...
const modelCapabilities = model => ({
  reasoning: /reason|thinking|claude|glm|gemini|gpt-4|qwen-max/i.test(String(model)),
  fast: /flash|mini|turbo|haiku/i.test(String(model))
})
...
const readCustomModels = () => {
  try {
    ...
    return rows
      ...
      .filter(row => row.id && row.base_url && row.model_name)
  } catch {
    return []
  }
}

const writeCustomModels = models => {
...
const publicCustomModel = row => ({
  ...row,
  provider: `custom:${row.id}`,
  api_key_env: customEnvKey(row.id),
  api_key_set: !!getEnvValue(customEnvKey(row.id)).trim(),
  api_key_redacted: maskSecret(getEnvValue(customEnvKey(row.id)).trim())
})

const customProviderFromRow = row => ({
  slug: `custom:${row.id}`,
...
const getCustomProviders = () => readCustomModels().map(customProviderFromRow).filter(p => p.authenticated)

const isImagePrompt = text => {
  const t = String(text || '').toLowerCase()
  const asciiHit = /image|picture|photo|draw|generate.*image|create.*image|poster|illustration|wallpaper|logo/.test(t)
  const zhWords = [
    '\u751f\u6210\u56fe',
    '\u751f\u6210\u56fe\u7247',
    '\u751f\u6210\u4e00\u5f20',
    ...
    '\u5c0f\u732b',
    '\u5c0f\u72d7'
  ].map(word => word.replace(/\\u([0-9a-fA-F]{4})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16))))
  return asciiHit || zhWords.some(word => t.includes(word))
}

const findUsableImageModel = () => readCustomModels().find(row => row.enabled !== false && row.usable === true && row.type === 'image') || null

const findUsableChatProvider = () => {
  const builtin = MODEL_PROVIDERS.find(isProviderConfigured)
  if (builtin) return { provider: builtin.slug, model: builtin.models[0] }
  const custom = readCustomModels().find(row => row.enabled !== false && row.usable === true && ['chat', 'vision'].includes(row.type || 'chat'))
  if (custom) return { provider: `custom:${custom.id}`, model: custom.model_name }
  return null
}

const routeModelForPrompt = (prompt, provider, model) => {
  ...
  if (isImagePrompt(prompt)) {
    if (row?.type === 'image') return { provider, model, row, routed: false }
    const imageRow = findUsableImageModel()
    if (imageRow) return { provider: `custom:${imageRow.id}`, model: imageRow.model_name, row: imageRow, routed: true }
  }

  if (row?.type === 'image' && !isImagePrompt(prompt)) {
    const chat = findUsableChatProvider()
    if (chat) return { ...chat, row: null, routed: true }
  }
...
let karnaConfig = {
  display: { language: 'zh', skin: 'karna', personality: '', show_reasoning: true },
...
let karnaConfigDefaults = cloneJson(karnaConfig)
...
const karnaConfigSchema = {
  fields: {
    ...
    'display.personality': configField('select', 'Default chat personality.', ['', 'karna']),
...
const logEntries = []
function rememberLog(msg) {
  const line = typeof msg === 'string' ? msg : String(msg)
  logEntries.push(line)
  if (logEntries.length > 500) logEntries.shift()
  // eslint-disable-next-line no-console
  console.log(`[karna-adapter] ${line}`)
}
function getRecentLogs() {
  return logEntries.slice(-80)
}
...
const testImageModelOnce = async ({ baseUrl, modelName, headers, image_size }) => {
  ...
  if (!output) return { ok: false, reachable: true, status: response.status, message: 'Image endpoint responded but no image URL/base64 was returned.' }
...
const testImageModel = async ({ base_url, model_name, api_key, image_size }) => {
  const baseUrl = String(base_url || '').trim().replace(/\/+$/, '')
  const modelName = String(model_name || '').trim()
  if (!baseUrl || !modelName) return { ok: false, reachable: false, message: 'base_url and model_name are required.' }
  const cleanKey = normalizeApiKey(api_key)
  ...
  try {
    ...
    // SiliconFlow has separate CN/COM API domains. Keys created on cloud.siliconflow.cn
    // commonly fail with 401 on api.siliconflow.com but work on api.siliconflow.cn.
    if (primary.status === 401 && /api\.siliconflow\.com/i.test(baseUrl)) {
      ...
      if (fallback.ok) return { ...fallback, message: 'Image model test passed on https://api.siliconflow.cn/v1. Saved CN endpoint automatically.' }
      return {
        ...fallback,
...
const recordArtifact = artifact => {
  const store = readBackendJson('artifacts.json', { version: 1, artifacts: [], settings: { markdown: true, image: true, file: true, code_block: true, download_link: true } })
  const rows = Array.isArray(store.artifacts) ? store.artifacts : []
  ...
  writeBackendJson('artifacts.json', { version: 1, settings: store.settings || {}, artifacts: [row, ...rows].slice(0, 500) })
  return row
}

const readArtifacts = () => {
  const store = readBackendJson('artifacts.json', { version: 1, settings: { markdown: true, image: true, file: true, code_block: true, download_link: true }, artifacts: [] })
  return { ...store, artifacts: Array.isArray(store.artifacts) ? store.artifacts : [] }
}

const generateImageWithCustomModel = async (row, prompt) => {
  ...
  const output = parseImageOutput(response.data)
  if (!output) throw new Error('Image generation succeeded but no image URL/base64 was returned.')
...
const testEmbeddingModel = async ({ base_url, model_name, api_key }) => {
  const baseUrl = String(base_url || '').trim().replace(/\/+$/, '')
  const modelName = String(model_name || '').trim()
  if (!baseUrl || !modelName) return { ok: false, reachable: false, message: 'base_url and model_name are required.' }
  const cleanKey = normalizeApiKey(api_key)
  ...
  try {
    ...
    if (!Array.isArray(embedding)) return { ok: false, reachable: true, status: response.status, message: 'Embedding endpoint responded but no embedding vector was returned.' }
...
const testCustomModelByType = async row => {
  if (row.type === 'image') return testImageModel(row)
  if (row.type === 'embedding') return testEmbeddingModel(row)
...
const upsertCustomModel = async (body, existingId = null) => {
  ...
  if (test.base_url) row.base_url = test.base_url
  row.last_test = { ...test, tested_at: now }
  if (!test.ok) return { ok: false, model: publicCustomModel(row), test, error: test.message, message: test.message }
  const next = models.filter(m => m.id !== id).concat(row)
  writeCustomModels(next)
  if (apiKey) writeEnvValue(customEnvKey(id), apiKey)
...
const readSkillsState = () => readBackendJson('skills_state.json', { version: 1, enabled: {} })
const writeSkillsState = state => writeBackendJson('skills_state.json', { version: 1, enabled: normalizeStateMap(state.enabled) })

const scanSkills = () => {
  const repoRoot = path.resolve(__dirname, '..', '..', '..')
  const roots = [path.join(repoRoot, 'skills', 'karna'), path.join(process.env.USERPROFILE || '', '.codex', 'skills'), path.join(process.env.USERPROFILE || '', '.agents', 'skills')]
  const state = readSkillsState()
  const enabledMap = normalizeStateMap(state.enabled)
  const rows = []
  const seen = new Set()
  const skillFiles = root => {
    const found = []
    const visit = dir => {
      let entries = []
      try {
  ...
  for (const root of roots) {
    for (const skillPath of skillFiles(root)) {
      try {
        ...
        rows.push({
          name: key,
          description: desc || `Local skill at ${skillPath}`,
...
const setSkillEnabled = (name, enabled) => {
  const skillName = String(name || '').trim()
  if (!skillName) return notConfigured('skills', 'Missing skill name.')
  const rows = scanSkills()
  const row = rows.find(item => item.name === skillName)
  if (!row) return notConfigured('skills', `Skill not found: ${skillName}`)
  const state = readSkillsState()
  state.enabled = normalizeStateMap(state.enabled)
  state.enabled[skillName] = enabled !== false
  writeSkillsState(state)
  return { ok: true, name: skillName, enabled: enabled !== false, path: row.path }
}

const getUserSkillRoot = () => path.join('D:\\Agent', 'skills')
const sanitizeSkillName = value => {
  const raw = String(value || '').trim().replace(/\s+/g, '-')
  const safe = raw.replace(/[<>:"/\\|?*\x00-\x1F]/g, '-').replace(/-+/g, '-').replace(/^-+|-+$/g, '')
  return safe || `skill-${Date.now()}`
}
const skillSearchScore = (skill, terms) => {
  const haystack = `${skill.name} ${skill.description} ${skill.category} ${skill.path}`.toLowerCase()
  return terms.reduce((score, term) => score + (haystack.includes(term) ? 2 : 0) + (skill.name.toLowerCase().includes(term) ? 3 : 0), 0)
}
...
const readSkillByName = name => {
  const skillName = String(name || '').trim()
  const skill = scanSkills().find(row => row.name === skillName)
  if (!skill) return notConfigured('skills', `Skill not found: ${skillName}`)
...
const createSkill = input => {
  ...
  if (!path.resolve(dir).startsWith(path.resolve(root))) return notConfigured('skills', 'Skill path escapes the user skill root.')
  if (fs.existsSync(skillPath)) return notConfigured('skills', `Skill already exists: ${name}`, { name, path: skillPath })
...
const readToolsetState = () => readBackendJson('toolsets.json', { version: 1, enabled: { mcp: true, skills: true, plugins: true, artifacts: true }, providers: {} })
const writeToolsetState = state => writeBackendJson('toolsets.json', { version: 1, enabled: normalizeStateMap(state.enabled), providers: normalizeStateMap(state.providers) })

const readPluginState = () => readBackendJson('plugins.json', { version: 1, enabled: {} })
const writePluginState = state => writeBackendJson('plugins.json', { version: 1, enabled: normalizeStateMap(state.enabled) })
...
const toolsetRows = () => {
  ...
  const artifacts = readArtifacts().artifacts
  return [
    { name: 'mcp', label: 'MCP 服务器', description: '读取本地 MCP 配置，显示连接状态和可用工具。', enabled: enabled.mcp !== false, configured: mcpServers.length > 0, tools: mcpServers.map(s => s.name).concat(['test_server', 'reload']) },
    { name: 'skills', label: 'Karna \u6280\u80fd', description: '\u626b\u63cf Karna \u5185\u7f6e\u548c\u672c\u5730\u6280\u80fd\u76ee\u5f55\uff0c\u652f\u6301\u542f\u7528\u3001\u7981\u7528\u548c\u8bfb\u53d6\u3002', enabled: enabled.skills !== false, configured: skills.length > 0, tools: skills.filter(s => s.enabled).map(s => s.name) },
    { name: 'plugins', label: 'Karna \u63d2\u4ef6', description: '\u8bfb\u53d6\u672c\u5730\u63d2\u4ef6\u6e05\u5355\uff0c\u663e\u793a\u542f\u7528\u72b6\u6001\u3002', enabled: enabled.plugins !== false, configured: plugins.length > 0, tools: plugins.filter(p => p.enabled).map(p => p.name) },
    { name: 'artifacts', label: '产物', description: '记录图片、文件、代码块和下载链接等真实产物。', enabled: enabled.artifacts !== false, configured: artifacts.length > 0, tools: artifacts.map(a => a.type || a.id) },
  ]
}

const toolsetConfig = name => {
  const state = readToolsetState()
  const providers = normalizeStateMap(state.providers)
  const rows = toolsetRows()
  const row = rows.find(item => item.name === name)
  if (!row) return { name, has_category: false, active_provider: null, providers: [], ok: false, capability: 'toolsets', error: `Unknown toolset: ${name}` }
  return {
    name,
    has_category: true,
...
const setToolsetEnabled = (name, enabled) => {
  const state = readToolsetState()
  state.enabled = normalizeStateMap(state.enabled)
...
const setToolsetProvider = (name, provider) => {
  const state = readToolsetState()
...
const scanPlugins = () => {
  const roots = [path.join(process.env.USERPROFILE || '', '.codex', 'plugins'), path.join(process.env.USERPROFILE || '', '.codex', 'plugins', 'cache')]
...
const setPluginEnabled = (id, enabled) => {
  ...
  if (!pluginId) return notConfigured('plugins', 'Missing plugin id.')
  const row = scanPlugins().find(item => item.id === pluginId)
  if (!row) return notConfigured('plugins', `Plugin not found: ${pluginId}`)
  const state = readPluginState()
  state.enabled = normalizeStateMap(state.enabled)
...
const writeKnowledgeBase = store => writeBackendJson('knowledge_base.json', store)
...
const knowledgeLibraryRows = store => {
  ...
  const baseRows = libraries.length ? libraries : folders
    ...
    .filter(row => !path.resolve(String(row.folder || '')).startsWith(writerRoot))
  return baseRows.map((lib, index) => {
    const resolved = path.resolve(String(lib.folder || lib.path || ''))
    if (resolved.startsWith(writerRoot)) return null
    const docs = (store.documents || []).filter(doc => String(doc.path || '').startsWith(resolved))
    const chunks = (store.chunks || []).filter(chunk => String(chunk.path || '').startsWith(resolved))
    return { id: lib.id || textHash(resolved).slice(0, 12), name: lib.name || path.basename(resolved) || `Knowledge ${index + 1}`, folder: resolved, vectorized: chunks.some(chunk => Array.isArray(chunk.embedding)), documents: docs.length, chunks: chunks.length, updated_at: lib.updated_at || store.last_indexed_at || null }
  }).filter(Boolean)
}
const writeKnowledgeLibraries = libraries => {
  const store = readKnowledgeBase()
  const folders = libraries.map(row => path.resolve(String(row.folder || row.path || ''))).filter(Boolean)
  writeKnowledgeBase({ ...store, libraries, config: { ...(store.config || {}), folders: Array.from(new Set(folders)) } })
  return readKnowledgeBase()
}
const upsertKnowledgeLibrary = ({ id = '', name = '', folder = '' } = {}) => {
  ...
  writeKnowledgeLibraries(next)
  return { ok: true, library: row, libraries: knowledgeLibraryRows(readKnowledgeBase()) }
}
const renameKnowledgeLibrary = (id, name) => {
  const store = readKnowledgeBase()
  const current = knowledgeLibraryRows(store)
  const next = current.map(row => row.id === id ? { ...row, name: String(name || row.name).trim(), updated_at: new Date().toISOString() } : row)
  writeKnowledgeLibraries(next)
  return { ok: true, libraries: knowledgeLibraryRows(readKnowledgeBase()) }
}
...
const getEmbeddingModelRow = preferredId => {
  const rows = readCustomModels().filter(row => row.enabled !== false && row.usable === true && row.type === 'embedding')
  if (preferredId) return rows.find(row => row.id === preferredId || `custom:${row.id}` === preferredId) || null
  return rows[0] || null
}

const embedTexts = async (texts, preferredId = '') => {
  const row = getEmbeddingModelRow(preferredId)
  if (!row) throw new Error('No usable embedding model configured. Add an Embedding model first in Model settings.')
  const apiKey = getEnvValue(customEnvKey(row.id)).trim()
  const cleanKey = normalizeApiKey(apiKey)
  const headers = cleanKey ? { Authorization: `Bearer ${cleanKey}` } : {}
  const baseUrl = String(row.base_url || '').trim().replace(/\/+$/, '')
  ...
  if (vectors.length !== texts.length) throw new Error(`Embedding endpoint returned ${vectors.length} vectors for ${texts.length} inputs.`)
...
const indexKnowledgeFolder = async (folder, options = {}) => {
  ...
  for (const file of files) {
    ...
    chunkMarkdown(content, Number(config.chunk_size) || 1200, Number(config.chunk_overlap) || 160).forEach((chunk, index) => {
      chunkRows.push({ id: textHash(`${docId}:${index}:${chunk}`), doc_id: docId, path: file, title: path.basename(file), index, text: chunk, hash: textHash(chunk), embedding: null })
    })
  }
  if (chunkRows.length > 0) {
    ...
    for (let i = 0; i < chunkRows.length; i += batchSize) {
      ...
      const result = await embedTexts(batch.map(row => row.text), config.embedding_model_id || '')
      embeddingModel = result.model
      result.vectors.forEach((vector, offset) => { batch[offset].embedding = vector })
    }
  ...
  } else if (!getEmbeddingModelRow(config.embedding_model_id || '')) {
    throw new Error('No usable embedding model configured. Add an Embedding model first in Model settings.')
  }
  const otherChunks = (store.chunks || []).filter(row => !String(row.path || '').startsWith(root))
  const otherDocs = (store.documents || []).filter(row => !String(row.path || '').startsWith(root))
  const folders = Array.from(new Set([...(Array.isArray(config.folders) ? config.folders : []), root]))
  const next = { version: 1, config: { ...config, folders }, documents: [...otherDocs, ...documents], chunks: [...otherChunks, ...chunkRows], last_indexed_at: new Date().toISOString() }
  writeKnowledgeBase(next)
  return { ok: true, folder: root, files: documents.length, chunks: chunkRows.length, embedding_model: config.embedding_model_name || config.embedding_model_id || null }
}
...
const searchKnowledge = async (query, opts = {}) => {
  ...
  const chunks = (Array.isArray(store.chunks) ? store.chunks : []).filter(row => row && row.text && (!projectRoot || String(row.path || '').startsWith(projectRoot)))
  const limit = Math.max(1, Math.min(20, Number(opts.limit || store.config?.top_k || 5)))
  if (!chunks.length) return { ok: true, query, results: [], vectorized: false, message: 'Knowledge base is empty.' }
  const vectorChunks = chunks.filter(row => Array.isArray(row.embedding))
  if (vectorChunks.length) {
    try {
      ...
      const results = vectorChunks.map(row => ({ ...row, score: cosineSimilarity(qv, row.embedding) })).sort((a, b) => b.score - a.score).slice(0, limit)
      return { ok: true, query, results, vectorized: true, embedding_model: embedded.model?.model_name || null }
    } catch (err) {
      if (!opts.allowLexical) throw err
    }
  }
  const results = chunks.map(row => ({ ...row, score: lexicalScore(query, row.text) })).filter(row => row.score > 0).sort((a, b) => b.score - a.score).slice(0, limit)
  return { ok: true, query, results, vectorized: false, warning: 'Vector search unavailable; used lexical fallback.' }
}

const knowledgeContextForPrompt = async (prompt, opts = {}) => {
  const store = readKnowledgeBase()
  if (store.config?.auto_inject === false) return ''
  const search = await searchKnowledge(prompt, { limit: store.config?.top_k || 5, allowLexical: true, projectRef: opts.projectRef })
...
const callBuiltinMcpTool = async (tool, args = {}) => {
  ...
  if (name === 'knowledge_search') return searchKnowledge(args.query || args.q || '', { limit: args.limit || 5, allowLexical: true })
  if (name === 'list_writer_skills') return { ok: true, skills: scanSkills().filter(skill => String(skill.path || '').includes(`${path.sep}skills${path.sep}karna${path.sep}writer${path.sep}`)) }
  if (name === 'read_skill') return readSkillByName(args.name || args.skill || '')
  if (name === 'find_skill') return searchSkills(args.query || args.q || args.keyword || '')
  if (name === 'create_skill') return createSkill(args)
...
const updateWorkflowRunNodeAction = (project, runId, nodeId, action, note = '') => {
  ...
  const statusByAction = { accept: 'accepted', reject: 'rejected', skip: 'skipped' }
...
const runWorkflowForProject = async (project, workflowIdText, input = {}, onlyNodeId = '') => {
  ...
  const runAgentNode = async (node, localUpstream) => {
    ...
    const knowledgeContext = (normalizeWorkflowPermissions(agent.permissions).canUseKnowledge && (node.data?.knowledge || workflow.knowledge_binding?.enabled))
      ...
      : ''
    const basePrompt = workflowNodePrompt({ project, workflow, node, agent, upstream: localUpstream, input: input.input || input.text || input.prompt || '' })
    const prompt = knowledgeContext ? `${basePrompt}\n\nRelevant knowledge excerpts:\n${knowledgeContext}\n\nUse the excerpts only when relevant and do not invent sources.` : basePrompt
    run.cost_estimate.calls += 1
    run.cost_estimate.tokens += Math.ceil(prompt.length / 3)
    let content = ''
    try {
      const nodeModel = String(node.data?.model || '').trim()
      const selectedModel = nodeModel && nodeModel !== '默认模型' ? nodeModel : (agent.model || input.model)
      const response = await karnaBackendFetch('/api/chat', { method: 'POST', body: { messages: [{ role: 'system', content: projectSessionContext(project, null) }, { role: 'user', content: prompt }], model: selectedModel, provider: input.provider }, timeoutMs: 300_000 })
    ...
    setNodeStatus(node, { status: content.startsWith('Node failed') ? 'blocked' : 'done', agent_id: agent.id, agent_name: agent.name, summary: content.slice(0, 1200) })
    return content
  }
...
const projectSessionContext = (project, agent = null) => {
  const base = [`Project: ${project.title}`, `Type: ${project.type}`, `Folder: ${project.folder}`]
...
const createStoredSession = ({ title = '\u65b0\u4f1a\u8bdd', cwd = '', project = null, agent = null, source = 'tui' } = {}) => {
...
const readTaskSystem = project => {
  ...
  catch { const data = defaultTaskSystem(project, readWriterAgents(project).agents); fs.writeFileSync(file, `${JSON.stringify(data, null, 2)}\n`, 'utf8'); return data }
}
...
const enrichWriterProject = project => {
  ...
  const agents = readWriterAgents(project).agents.map(agent => ({ ...agent, ...agentStatusFromTasks(agent, tasks.tasks || []) }))
  const controller = agents.find(agent => agent.id === 'controller') || { id: 'controller', name: '\u4e3b\u63a7', role: '\u4e3b\u63a7', brief: '\u7edf\u7b79\u9879\u76ee\u8ba1\u5212\u3001\u4efb\u52a1\u5206\u53d1\u548c\u8fdb\u5ea6\u76d1\u63a7' }
  if (project.main_session_id) ensureStoredSession({ id: project.main_session_id, title: `${project.title} \u00b7 \u4e3b\u63a7`, cwd: project.folder, project, agent: controller })
  for (const agent of agents) {
    if (agent.session_id) ensureStoredSession({ id: agent.session_id, title: agent.id === 'controller' ? `${project.title} \u00b7 \u4e3b\u63a7` : `${agent.name} \u00b7 ${project.title}`, cwd: project.folder, project, agent })
  }
  const projectSessions = Array.from(sessions.values()).filter(session => session.project_id === project.id).map(storedSessionInfo)
  return { ...project, session_ids: Array.isArray(project.session_ids) ? project.session_ids : projectSessions.map(s => s.id), agent_session_ids: project.agent_session_ids || Object.fromEntries(agents.filter(a => a.session_id).map(a => [a.id, a.session_id])), agents, tasks, sessions: projectSessions }
}
...
const slugify = text => {
  const raw = String(text || 'untitled').trim()
...
const ensureWriterProjectFolders = project => {
  const folders = ['manuscript', 'notes', 'characters', 'world', 'research', 'drafts', 'exports', 'workflow_artifacts', 'imports', 'bible', 'analysis', 'versions', 'privacy']
  fs.mkdirSync(project.folder, { recursive: true })
  for (const name of folders) fs.mkdirSync(path.join(project.folder, name), { recursive: true })
}
const upsertKnowledgeFolder = folder => {
  const store = readKnowledgeBase()
  const folders = Array.from(new Set([...(Array.isArray(store.config?.folders) ? store.config.folders : []), folder]))
  writeKnowledgeBase({ ...store, config: { ...(store.config || {}), folders } })
}
...
const writerAgentTemplates = type => {
  ...
  const sets = {
    'novel': ['\u4e3b\u7b14', '\u5267\u60c5\u7f16\u8f91', '\u4eba\u7269\u7f16\u8f91', '\u4e16\u754c\u89c2\u7ba1\u7406\u5458', '\u8d44\u6599\u5458', '\u6821\u5bf9'],
    'web-novel': ['\u7f51\u6587\u4e3b\u7b14', '\u723d\u70b9\u8282\u594f\u7f16\u8f91', '\u4eba\u7269\u7f16\u8f91', '\u4e16\u754c\u89c2\u7ba1\u7406\u5458', '\u8d44\u6599\u5458', '\u8ffd\u66f4\u6821\u5bf9'],
    paper: ['\u7814\u7a76\u52a9\u7406', '\u7ed3\u6784\u7f16\u8f91', '\u5f15\u7528\u68c0\u67e5', '\u6458\u8981\u6da6\u8272', '\u5ba1\u7a3f\u4eba\u6a21\u62df'],
    screenplay: ['\u7f16\u5267', '\u573a\u666f\u8c03\u5ea6', '\u5bf9\u767d\u533b\u751f', '\u8282\u594f\u7f16\u8f91'],
    copywriting: ['\u521b\u610f\u7b56\u5212', '\u98ce\u683c\u7f16\u8f91', '\u4e8b\u5b9e\u68c0\u67e5', '\u7ec8\u7a3f\u6821\u5bf9'],
    poetry: ['\u8bd7\u6b4c\u5de5\u574a', '\u610f\u8c61\u7f16\u8f91', '\u8282\u594f\u6821\u5bf9', '\u98ce\u683c\u7f16\u8f91'],
    editorial: ['\u8d23\u4efb\u7f16\u8f91', '\u4e8b\u5b9e\u68c0\u67e5', '\u7ed3\u6784\u7f16\u8f91', '\u7ec8\u7a3f\u6821\u5bf9']
  }
  const editorRoles = ['\u4e3b\u7f16', '\u8bbe\u5b9a\u7f16\u8f91', '\u4eba\u8bbe\u7f16\u8f91', '\u8282\u594f\u7f16\u8f91', '\u5bf9\u767d\u7f16\u8f91', '\u4f0f\u7b14\u7f16\u8f91', '\u8bfb\u8005\u53cd\u9988']
  const roles = editorRoles
  return { ...common, type: t, agents: roles.map((role, index) => ({ id: `${t}-editor-${index + 1}`, role, enabled: true, brief: `${role}\u53ea\u505a\u521b\u4f5c\u5de5\u7a0b\u7ba1\u7406\u3001\u4e00\u81f4\u6027\u68c0\u67e5\u548c\u53ef\u5ba1\u9605\u5efa\u8bae\uff0c\u4e0d\u66ff\u4f5c\u8005\u81ea\u52a8\u5b9a\u7a3f\u3002` })) }
}
...
const enhancePromptText = input => {
  const text = String(input?.text || input?.prompt || input || '').trim()
  const mode = String(input?.mode || 'chat')
  const resources = []
  if (Array.isArray(input?.skills) && input.skills.length) resources.push(`\u6280\u80fd\uff1a${input.skills.join('\u3001')}`)
  if (Array.isArray(input?.mcp) && input.mcp.length) resources.push(`MCP/\u5de5\u5177\uff1a${input.mcp.join('\u3001')}`)
  if (Array.isArray(input?.knowledge) && input.knowledge.length) resources.push(`\u77e5\u8bc6\u5e93\uff1a${input.knowledge.join('\u3001')}`)
  const modeLine = mode === 'plan' ? '\u8bf7\u5148\u62c6\u89e3\u8ba1\u5212\uff0c\u5173\u952e\u5206\u6b67\u7ed9\u51fa\u9009\u9879\u7b49\u6211\u786e\u8ba4\u3002' : mode === 'goal' ? '\u8bf7\u56f4\u7ed5\u6700\u7ec8\u76ee\u6807\u6301\u7eed\u63a8\u8fdb\uff0c\u81ea\u52a8\u62c6\u4efb\u52a1\u5e76\u68c0\u67e5\u8fdb\u5ea6\u3002' : '\u8bf7\u7ed9\u51fa\u53ef\u6267\u884c\u3001\u53ef\u9a8c\u8bc1\u7684\u7ed3\u679c\u3002'
  return { ok: true, text: `${text}\n\n\u8865\u5145\u8981\u6c42\uff1a\n- ${modeLine}\n- \u5982\u6d89\u53ca\u4eba\u7269\u5173\u7cfb\u3001\u4e8b\u4ef6\u8054\u7cfb\u3001\u77e5\u8bc6\u56fe\u8c31\uff0c\u8bf7\u8f93\u51fa\u81ea\u7136\u8bed\u8a00\u8bf4\u660e\u548c\u5bb9\u9519 JSON\u3002${resources.length ? `\n- \u53ef\u7528\u8d44\u6e90\uff1a${resources.join('\uff1b')}` : ''}` }
}
...
const resolveProjectRoot = input => {
  const root = String(input?.root || input?.projectRoot || input?.folder || '').trim()
  return root ? path.resolve(root) : getWriterProjectsRoot()
}
const createWriterProject = async input => {
  const { title, type: parsedType } = parseTypedTitle(input?.title || input?.arg || input)
  const type = normalizeProjectType(input?.type || parsedType)
  const store = readWriterProjects()
  ...
  const workspaceMode = input?.workspaceMode || input?.workspace_mode || 'new'
  const selectedRoot = String(input?.root || input?.projectRoot || input?.folder || '').trim()
  const root = workspaceMode === 'existing' && selectedRoot ? path.resolve(selectedRoot) : resolveProjectRoot(input || {})
  const folder = workspaceMode === 'existing' && selectedRoot ? root : path.join(root, slug)
  const now = new Date().toISOString()
  ...
  const project = { id, slug, title, type, folder, root, created_at: now, updated_at: now, status: 'active', pinned: Boolean(input?.pinned), multi_agent_enabled: multiAgentEnabled, coordination_mode: coordinationMode, session_ids: [], main_session_id: '', agent_session_ids: {}, knowledge_ids: Array.isArray(input?.knowledgeIds) ? input.knowledgeIds.map(String) : [] }
  ensureWriterProjectFolders(project)
  ensureWriterProjectMetadata(project)
  fs.writeFileSync(path.join(folder, 'project.md'), `# ${title}\n\n- Type: ${type}\n- Created: ${now}\n- Folder: ${folder}\n\n## Goal\n\n${String(input?.goal || '').trim()}\n`, 'utf8')
  if (!fs.existsSync(path.join(folder, 'outline.md'))) fs.writeFileSync(path.join(folder, 'outline.md'), writerOutlineMarkdown(title), 'utf8')
  if (!fs.existsSync(path.join(folder, 'canon.md'))) fs.writeFileSync(path.join(folder, 'canon.md'), `# ${title} Canon\n\n`, 'utf8')
  ...
  const agentsInput = multiAgentEnabled ? [controller, ...(inputAgents || writerAgentTemplates(input?.agentTemplate || type).agents)] : [controller]
  const agents = writeWriterAgents(project, input?.agentTemplate || type, agentsInput)
  const mainSession = createStoredSession({ title: `${title} ? \u4e3b\u63a7`, cwd: folder, project, agent: controller })
  project.main_session_id = mainSession.id
  project.session_ids.push(mainSession.id)
  project.agent_session_ids[controller.id] = mainSession.id
  agents.agents = agents.agents.map(agent => agent.id === controller.id ? { ...agent, session_id: mainSession.id } : agent)
  if (multiAgentEnabled) {
    agents.agents = agents.agents.map(agent => {
      if (agent.id === controller.id || agent.enabled === false) return agent
      const session = createStoredSession({ title: `${agent.name} ? ${title}`, cwd: folder, project, agent })
      project.session_ids.push(session.id)
      project.agent_session_ids[agent.id] = session.id
      return { ...agent, session_id: session.id }
    })
  }
  writeWriterAgentsData(project, agents.agents, { template: agents.template, enabled: multiAgentEnabled, concurrency: { enabled: multiAgentEnabled, max_parallel: Math.max(1, Number(input?.maxParallelAgents || 3)) } })
  ...
  karnaConfig = { ...karnaConfig, terminal: { ...(karnaConfig.terminal || {}), cwd: folder } }
  const knowledge = { ok: false, folder, status: 'not_selected', folders: [], selected: project.knowledge_ids }
  ...
  if (knowledgeFolders.length) {
    ...
    const embed = getEmbeddingModelRow(readKnowledgeBase().config?.embedding_model_id || '')
    if (embed) {
      try {
        const indexedRows = []
        for (const targetFolder of knowledgeFolders) indexedRows.push(await indexKnowledgeFolder(targetFolder, readKnowledgeBase().config || {}))
      ...
      knowledge.vectorization = notConfigured('knowledge', 'Knowledge is bound, but no Embedding model is configured; vectorization is skipped.', { status: 'not_configured' })
    }
  }
  ...
  return { ...enrichWriterProject(project), knowledge, import_result, agents: readWriterAgents(project).agents, tasks: readTaskSystem(project), workspace: { cwd: folder, configured: true }, next_commands: ['/chapter', '/char-save', '/world-save', '/research-save', '/find-skill'] }
}
const findWriterProject = ref => {
  const store = readWriterProjects()
  const rows = Array.isArray(store.projects) ? store.projects : []
  const key = String(ref || '').trim()
  if (key) return rows.find(p => p.id === key || p.slug === key || p.title === key) || null
...
const exportWriterProject = projectRef => {
  ...
  const parts = [`# ${project.title}\n\n_Type: ${project.type}_\n`]
  for (const file of files) parts.push(`## ${path.relative(project.folder, file)}\n\n${fs.readFileSync(file, 'utf8')}`)
  ...
  writeJsonFile(jsonFile, { exported_at: new Date().toISOString(), project, bible, versions: versions.entries || [], source_files: files.map(file => ({ file: path.relative(project.folder, file), content: fs.readFileSync(file, 'utf8') })) })
  ...
  appendWriterProjectVersion(project, 'export', `Exported ${files.length} chapters plus JSON data package`, { file: exportFile, json: jsonFile })
  return { project, file: exportFile, json: jsonFile, sources: files.length, artifact }
}
...
const ensureWriterProjectMetadata = project => {
  ...
  const privacyFile = path.join(project.folder, 'privacy', 'README.md')
  if (!fs.existsSync(privacyFile)) fs.writeFileSync(privacyFile, writerProjectPrivacyMarkdown(project), 'utf8')
  if (!fs.existsSync(writerProjectVersionsPath(project))) writeJsonFile(writerProjectVersionsPath(project), { version: 1, project_id: project.id, entries: [] })
}
...
const readProjectDocuments = project => projectSourceFiles(project).map(file => {
...
const defaultSoulProfile = name => ({ version: 1, author_name: name, updated_at: null, core_worldview: [], common_themes: [], narrative_methods: [], character_design_patterns: [], dialogue_features: [], sentence_features: [], imagery_system: [], conflict_patterns: [], pacing_preferences: [], critical_lens: [], do_not_copy: [], safe_transfer_principles: [], evidence_refs: [] })
const enrichSoulAuthor = author => {
  ...
  const webClaims = soulJson(author, 'web_research/extracted_claims.json', { items: [] }).items || []
  const profile = soulJson(author, 'soul_profile.json', defaultSoulProfile(author.name))
  return { ...author, texts_count: (meta.texts || []).length, chunks_count: chunks.length, web_evidence_count: webClaims.length, profile_version: profile.updated_at ? profile.version || 1 : 0, profile_updated_at: profile.updated_at || null }
}
const listSoulAuthors = () => {
  const store = readSoulStore()
  const authors = (store.authors || []).map(enrichSoulAuthor)
  return { ok: true, authors, active_author_id: store.active_author_id || authors[0]?.id || '' }
}
const findSoulAuthor = ref => {
  const store = readSoulStore()
  const rows = Array.isArray(store.authors) ? store.authors : []
  const key = String(ref || '').trim()
  if (key) return rows.find(a => a.id === key || a.slug === key || a.name === key) || null
  return rows.find(a => a.id === store.active_author_id) || rows[0] || null
}
const createSoulAuthor = input => {
  ...
  const author = { id: soulId('author'), slug, name, created_at: soulNow(), updated_at: soulNow(), folder: path.join(soulAuthorsRoot(), slug), language: input?.language || 'zh', status: 'active' }
  ensureSoulAuthorFolders(author)
...
const importSoulTexts = (ref, input = {}) => {
  ...
  updateSoulAuthorInStore(author)
  soulLog(author, 'import', { sent_scope: 'local-copy-and-parse', note: `${imported.length} files imported` })
  return { ok: true, author: enrichSoulAuthor(author), imported }
}
...
const sourceCredibility = url => {
  ...
  let score = 0.45
  if (/\.edu|\.ac\.|university|press|journal|jstor|doi|official|gov|org/.test(u)) score += 0.3
  if (/baike\.baidu|wikipedia|britannica|douban|goodreads|nytimes|newyorker|guardian|bbc/.test(u)) score += 0.15
  if (/blog|forum|tieba|reddit|zhihu/.test(u)) score -= 0.1
...
const webResearchSoulAuthor = async (ref, input = {}) => {
  ...
  for (const q of queries) {
    queryRows.push({ query: q, at: soulNow() })
    const results = await webSearchPublic(q)
    if (!results.length) warnings.push({ query: q, message: '\u641c\u7d22\u6e90\u8d85\u65f6\u6216\u672a\u8fd4\u56de\u53ef\u7528\u7ed3\u679c\uff0c\u5df2\u8df3\u8fc7\u8be5\u67e5\u8be2\u3002' })
    for (const row of results) {
      const copyright_risk = new RegExp('txt|read|novel|chapter|fulltext|\\u5168\\u96c6|\\u5728\\u7ebf\\u9605\\u8bfb|\\u76d7\\u7248', 'i').test(`${row.url} ${row.title}`) ? 'high' : 'low'
      const credibility = sourceCredibility(row.url)
      const source = { id: soulId('source'), query: q, title: row.title, url: row.url, summary: row.snippet, credibility, copyright_risk, saved_at: soulNow() }
      allSources.push(source)
      if (copyright_risk !== 'high') allClaims.push({ id: soulId('claim'), claim: summarizeText(row.snippet || row.title, 220), evidence: row.snippet, source_url: row.url, source_title: row.title, confidence: credibility, category: new RegExp('interview|\\u8bbf\\u8c08|lecture|talk', 'i').test(`${row.title} ${row.snippet}`) ? 'narrative_method' : 'critical_lens' })
    }
  }
...
const readSoulAuthorDetail = ref => {
  const author = findSoulAuthor(ref)
  if (!author) throw new Error(`Soul author not found: ${ref}`)
  return { ok: true, author: enrichSoulAuthor(author), metadata: soulJson(author, 'metadata.json', { texts: [] }), chunks: readSoulChunks(author).slice(0, 80), profile: soulJson(author, 'soul_profile.json', defaultSoulProfile(author.name)), risk_profile: soulJson(author, 'risk_profile.json', { checks: [] }), web: { sources: soulJson(author, 'web_research/sources.json', { items: [] }).items || [], claims: soulJson(author, 'web_research/extracted_claims.json', { items: [] }).items || [], conflicts: soulJson(author, 'web_research/conflicts.json', { conflicts: [] }).conflicts || [] } }
}
...
const appendCanonNote = async note => {
  const store = readKnowledgeBase()
  const folder = store.config?.folders?.[0] || getKnowledgeDefaultFolder()
  fs.mkdirSync(folder, { recursive: true })
  const file = path.join(folder, 'karna-canon-inbox.md')
  const entry = `\n\n## ${new Date().toISOString()}\n\n${String(note || '').trim() || '(empty canon note)'}\n`
  fs.appendFileSync(file, entry, 'utf8')
  try {
    const indexed = await indexKnowledgeFolder(folder, store.config || {})
...
const executeWriterSlash = async input => {
  ...
  if (name === 'kb-reindex') {
    const store = readKnowledgeBase()
    const folders = Array.isArray(store.config?.folders) && store.config.folders.length ? store.config.folders : [getKnowledgeDefaultFolder()]
    const rows = []
    for (const folder of folders) rows.push(await indexKnowledgeFolder(folder, store.config || {}))
    return { output: rows.map(row => `${row.folder}: ${row.files} files, ${row.chunks} chunks`).join('\n') || 'No knowledge folders configured.' }
  }
  if (name === 'writer-skills') return { output: scanSkills().filter(skill => String(skill.path || '').includes(`${path.sep}skills${path.sep}karna${path.sep}writer${path.sep}`)).map(skill => `- ${skill.name}: ${skill.description}`).join('\n') }
  if (name === 'skill-read') {
    const result = readSkillByName(arg)
    return { output: result.ok ? result.content : result.error }
  }
  if (name === 'find-skill') {
    const result = searchSkills(arg)
  ...
  if (name === 'create-skill') {
    ...
    const result = createSkill({ name: rawName, description: rawDesc || `用户创建的技能：${rawName}`, instructions: rest.join(' :: ') })
...
const completeWriterSlash = text => {
  ...
  if (token.startsWith('find-skill ')) return { items: searchSkills(raw.replace(/^\//, '').replace(/^find-skill\s+/i, '')).skills.map(skill => ({ text: skill.name, display: skill.name, meta: skill.description || skill.path })), replace_from: '/find-skill '.length }
  if (token.startsWith('skill-read ')) {
    const arg = raw.replace(/^\//, '').replace(/^skill-read\s+/i, '').toLowerCase()
    const items = scanSkills().filter(skill => skill.name.toLowerCase().includes(arg)).slice(0, 20).map(skill => ({ text: skill.name, display: skill.name, meta: skill.description || skill.path }))
  ...
  switch (method) {
    ...
    case 'session.resume': {
      ...
      if (session) {
        ...
        sendGatewayEvent(socket, sid, 'session.info', sessionInfoPayload(newSession))
        sendJsonRpcResult(socket, id, { resumed: sid, session_id: sid, message_count: 0, info: sessionInfoPayload(newSession), messages: [] })
      }
      break
    }
    ...
    case 'config.get': {
      sendJsonRpcResult(socket, id, { ...karnaConfig, mcp_servers: readMcpServers() })
      break
    }
    ...
    case 'config.set': {
      if (p.key === 'model' && typeof p.value === 'string') {
        ...
        karnaConfig = { ...karnaConfig, ...restConfig }
      }
      sendJsonRpcResult(socket, id, { ok: true })
      break
    }
...
const runAgentTask = async ({ socket, project, agent, tasks, mode, model, provider }) => {
  ...
  const response = await karnaBackendFetch('/api/chat', { method: 'POST', body: { messages, model, provider }, timeoutMs: 300_000 })
...
async function handlePromptSubmit(socket, id, params) {
  ...
  const messages = [
    ...(projectContext ? [{ role: 'system', content: projectContext }] : []),
    ...(knowledgeContext ? [{ role: 'system', content: `Relevant Karna knowledge base excerpts:
...
Use these excerpts when relevant. Do not invent sources beyond them.` }] : []),
  ...
  try {
    ...
    if (customRow?.type === 'image') {
      const content = await generateImageWithCustomModel(customRow, prompt)
      response = { status: 200, data: { content, provider, model } }
    } else {
      response = await karnaBackendFetch('/api/chat', {
        method: 'POST',
...
async function handleKarnaApiRequest(request) {
  ...
  if (reqPath === '/api/sessions' || reqPath.startsWith('/api/sessions?')) {
    if (method === 'GET') {
      const sessionList = Array.from(sessions.values()).map(storedSessionInfo).sort((a, b) => b.last_active - a.last_active)
  ...
  if (reqPath === '/api/config') {
    if (method === 'GET') return { ...karnaConfig, mcp_servers: readMcpServers() }
    if (method === 'PUT') {
      if (body?.config) {
  ...
  if (reqPath === '/api/config/defaults') return karnaConfigDefaults
  if (reqPath === '/api/config/schema') {
    return karnaConfigSchema
  }

  // ---- Model endpoints (mock) ----
  ...
  if (reqPath === '/api/model/custom') {
    ...
    if (method === 'POST') return upsertCustomModel(body)
  }
  ...
  if (customModelMatch) {
    ...
    if (method === 'DELETE') {
      const next = readCustomModels().filter(m => m.id !== id)
      writeCustomModels(next)
      deleteEnvValue(customEnvKey(id))
      if (currentModelProvider === `custom:${id}`) ensureCurrentConfiguredProvider()
      return { ok: true }
    }
    if (method === 'PUT') return upsertCustomModel(body, id)
    if (method === 'POST' && customModelMatch[2] === 'test') {
      const row = readCustomModels().find(m => m.id === id)
      if (!row) return { ok: false, reachable: false, message: 'Custom model not found.' }
      const apiKey = String(body?.api_key || '').trim() || getEnvValue(customEnvKey(id)).trim()
      const test = await testCustomModelByType({ ...row, base_url: body?.base_url || row.base_url, model_name: body?.model_name || row.model_name, api_key: apiKey })
      const rows = readCustomModels().map(m => m.id === id ? { ...m, usable: test.ok === true, last_test: { ...test, tested_at: new Date().toISOString() }, updated_at: new Date().toISOString() } : m)
  ...
  // ---- Skills ----
  if (reqPath === '/api/skills') return scanSkills()
  if (reqPath.startsWith('/api/skills/search')) { const url = new URL(reqPath, 'http://karna.local'); return searchSkills(url.searchParams.get('q') || body?.q || body?.query || '') }
  if (reqPath === '/api/skills/create' && method === 'POST') return createSkill(body || {})
  const skillReadMatch = reqPath.match(/^\/api\/skills\/([^/?]+)$/)
  if (skillReadMatch && method === 'GET') return readSkillByName(decodeURIComponent(skillReadMatch[1]))
  if (reqPath === '/api/skills/toggle') return setSkillEnabled(body?.name, body?.enabled !== false)
  ...
  if (reqPath === '/api/artifacts') {
    ...
    if (method === 'PUT') {
      ...
      writeBackendJson('artifacts.json', { ...store, settings: { ...(store.settings || {}), ...(body?.settings || {}) } })
      return { ok: true, settings: { ...(store.settings || {}), ...(body?.settings || {}) } }
    }
  }
  ...
  if (artifactMatch) {
    ...
    if (method === 'DELETE') {
      writeBackendJson('artifacts.json', { ...store, artifacts: store.artifacts.filter(item => item.id !== artifactId) })
  ...
  if (reqPath === '/api/knowledge') {
    if (method === 'GET') { const store = readKnowledgeBase(); return { ...store, libraries: knowledgeLibraryRows(store) } }
    if (method === 'PUT') {
      const store = readKnowledgeBase()
      const config = { ...(store.config || {}), ...(body?.config || body || {}) }
      writeKnowledgeBase({ ...store, config })
      return { ok: true, config, libraries: knowledgeLibraryRows(readKnowledgeBase()) }
    }
  }
  if (reqPath === '/api/knowledge/libraries') {
    if (method === 'GET') return { ok: true, libraries: knowledgeLibraryRows(readKnowledgeBase()) }
  ...
  if (reqPath === '/api/knowledge/import-folder' && method === 'POST') {
    try { const row = upsertKnowledgeLibrary({ name: body?.name, folder: body?.path || body?.folder }); const indexed = await indexKnowledgeFolder(body?.path || body?.folder, body || {}); return { ...indexed, library: row.library, libraries: knowledgeLibraryRows(readKnowledgeBase()) } }
  ...
  if (reqPath === '/api/knowledge/search' && method === 'POST') {
    try { return await searchKnowledge(body?.query || body?.q || '', { limit: body?.limit, allowLexical: true }) }
  ...
  if (builtinMcpCallMatch && method === 'POST') return callBuiltinMcpTool(decodeURIComponent(builtinMcpCallMatch[1]), body || {})
  ...
  if (reqPath === '/api/writer/resources') {
    const skills = scanSkills().map(skill => ({ id: skill.name, name: skill.name, description: skill.description || '', path: skill.path, enabled: skill.enabled !== false }))
    const mcp = Object.values(readMcpServers()).map(server => ({ id: server.name, name: server.name, description: server.description || '', enabled: server.enabled !== false, tools: server.tools || [] }))
    const plugins = scanPlugins().map(plugin => ({ id: plugin.id, name: plugin.name || plugin.id, description: plugin.description || '', path: plugin.path, enabled: plugin.enabled !== false }))
    return { ok: true, skills, mcp, plugins, knowledge: knowledgeLibraryRows(readKnowledgeBase()) }
  }
  ...
  if (reqPath === '/api/prompt/enhance' && method === 'POST') {
    try { return enhancePromptText(body || {}) }
  ...
  const workflowRunNodeActionMatch = reqPath.match(/^\/api\/writer\/workflows\/([^/?]+)\/runs\/([^/?]+)\/nodes\/([^/?]+)\/(accept|reject|skip)(?:\?|$)/)
  ...
  if (reqPath === '/api/soul/authors' || reqPath.startsWith('/api/soul/authors?')) {
    try {
      if (method === 'GET') return listSoulAuthors()
  ...
  const soulAuthorMatch = reqPath.match(/^\/api\/soul\/authors\/([^/?]+)(?:\/(import|process|search|web-research|distill|critic|risk-check|export-skill|export|detail))?$/)
  if (soulAuthorMatch) {
  ...
  if (reqPath === '/api/writer/projects' || reqPath.startsWith('/api/writer/projects?')) {
    ...
    if (method === 'POST') {
      try { return { ok: true, project: await createWriterProject(body || {}) } }
      catch (err) { return notConfigured('writer_projects', err instanceof Error ? err.message : String(err)) }
    }
  }
  ...
  if (reqPath === '/api/logs' || reqPath.startsWith('/api/logs?')) {
    return { file: 'desktop', lines: getRecentLogs() }
  }
  ...
  if (profileSoulMatch) {
    ...
    if (method === 'GET') {
      const content = profileSouls.get(profileName) || ''
      return { content, exists: content.length > 0, profile: profileName }
    }
    if (method === 'PUT') {
      profileSouls.set(profileName, String(body?.content || ''))
  ...
  if (profileMatch) {
    ...
    if (method === 'PUT') {
      ...
      if (profileSouls.has(profileName)) {
        profileSouls.set(nextName, profileSouls.get(profileName) || '')
        profileSouls.delete(profileName)
      }
  ...
  if (reqPath === '/api/analytics' || reqPath.startsWith('/api/analytics')) {
    return notConfigured('analytics', 'Analytics storage is not configured.', { totals: { sessions: 0, messages: 0, tokens: 0 }, daily: [], models: [], skills: [] })
  }
